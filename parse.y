...
function_decl		: function_head SEMI sub_routine SEMI 					{ $$ = makeFunDcl($1, $3); }
					;
function_head		: FUNCTION ID parameters COLON simple_type_decl			{ $$ = instFun(link($1, link($2, link($5, $3)))); } // FUNCTION, ID, type, para 
					;
procedure_decl		: procedure_head SEMI sub_routine SEMI 					{ $$ = makeFunDcl($1, $3); }
					;
procedure_head		: PROCEDURE ID parameters 								{ $$ = instFun(link($1, link($2, $3))); }	// PROCEDURE, ID, para 
					;
